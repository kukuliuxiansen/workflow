    ...
    async function deleteSelectedNode() {
      ...
      if (!await confirmAsync('确定删除此节点？')) return;
    ...
    function updateLogCounts() {
      ...
      if (execLogCount && state.logs.execution) execLogCount.textContent = state.logs.execution.length;
      if (agentLogCount && state.logs.agent) agentLogCount.textContent = state.logs.agent.length;
      if (apiLogCount && state.logs.api) apiLogCount.textContent = state.logs.api.length;
