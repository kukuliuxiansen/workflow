#!/usr/bin/env python3
"""
Workflows 项目历史功能测试脚本
测试内容：
1. 打开页面并检查工作流列表
2. 测试执行历史面板
3. 测试执行、暂停、恢复功能
4. 检查页面内容的实时更新
"""
from playwright.sync_api import sync_playwright
import time
import json
import os

TEST_URL = "http://localhost:3001"
SCREENSHOT_DIR = "/tmp/workflow_test_screenshots"

def ensure_dir(path):
    if not os.path.exists(path):
        os.makedirs(path)

def save_screenshot(page, name):
    path = f"{SCREENSHOT_DIR}/{name}.png"
    page.screenshot(path=path)
    print(f"  截图保存: {path}")
    return path

def test_workflows_history():
    """主测试函数"""
    ensure_dir(SCREENSHOT_DIR)

    issues = []  # 记录发现的问题

    print("\n" + "="*60)
    print("Workflows 历史功能测试")
    print("="*60)

    with sync_playwright() as p:
        browser = p.chromium.launch(headless=False)
        context = browser.new_context()
        page = context.new_page()

        # 监听控制台消息
        console_messages = []
        page.on('console', lambda msg: console_messages.append({
            'type': msg.type,
            'text': msg.text
        }))

        # 监听页面错误
        page_errors = []
        page.on('pageerror', lambda err: page_errors.append(str(err)))

        try:
            # ========================================
            # 步骤1: 打开页面
            # ========================================
            print("\n[步骤1] 打开页面...")
            page.goto(TEST_URL, timeout=10000)
            time.sleep(2)
            save_screenshot(page, "01_initial_page")

            # 检查工作流列表是否加载
            workflow_list = page.locator("#workflowList")
            if workflow_list.count() == 0:
                issues.append("工作流列表未加载")
            else:
                print(f"  工作流列表已加载")

            # ========================================
            # 步骤2: 点击历史按钮
            # ========================================
            print("\n[步骤2] 打开执行历史面板...")

            # 查找历史按钮
            history_btn = page.locator("button:has-text('历史')")
            if history_btn.count() > 0:
                history_btn.click()
                time.sleep(1)
                save_screenshot(page, "02_history_panel")

                # 检查历史面板是否显示
                history_panel = page.locator("#historyPanel")
                if not history_panel.is_visible():
                    issues.append("历史面板未显示")
                else:
                    print("  历史面板已显示")

                    # 检查历史列表
                    history_list = page.locator("#historyList .history-item")
                    history_count = history_list.count()
                    print(f"  发现 {history_count} 条历史记录")

                    if history_count == 0:
                        print("  警告: 没有历史记录，需要先执行工作流")
            else:
                issues.append("未找到历史按钮")

            # ========================================
            # 步骤3: 选择一个工作流
            # ========================================
            print("\n[步骤3] 选择工作流...")

            # 先关闭历史面板
            close_btn = page.locator("#historyPanel .modal-close")
            if close_btn.count() > 0 and close_btn.is_visible():
                close_btn.click()
                time.sleep(0.5)

            # 选择"商角AI"工作流或第一个工作流
            workflow_items = page.locator("#workflowList .workflow-item")
            if workflow_items.count() > 0:
                first_item = workflow_items.first
                workflow_name = first_item.locator(".workflow-name").text_content() if first_item.locator(".workflow-name").count() > 0 else "未命名"
                print(f"  选择工作流: {workflow_name}")
                first_item.click()
                time.sleep(1)
                save_screenshot(page, "03_selected_workflow")
            else:
                issues.append("没有可用的工作流")

            # ========================================
            # 步骤4: 检查执行按钮状态
            # ========================================
            print("\n[步骤4] 检查执行按钮状态...")

            btn_execute = page.locator("#btnExecute")
            btn_pause = page.locator("#btnPause")
            btn_resume = page.locator("#btnResume")

            execute_visible = btn_execute.is_visible() if btn_execute.count() > 0 else False
            pause_visible = btn_pause.is_visible() if btn_pause.count() > 0 else False
            resume_visible = btn_resume.is_visible() if btn_resume.count() > 0 else False

            print(f"  执行按钮可见: {execute_visible}")
            print(f"  暂停按钮可见: {pause_visible}")
            print(f"  继续按钮可见: {resume_visible}")

            if not execute_visible and not pause_visible and not resume_visible:
                issues.append("所有执行按钮都不可见")

            # ========================================
            # 步骤5: 配置任务并执行
            # ========================================
            print("\n[步骤5] 配置任务并执行...")

            # 打开任务配置
            task_config_btn = page.locator("button:has-text('任务配置')")
            if task_config_btn.count() > 0:
                task_config_btn.click()
                time.sleep(0.5)
                save_screenshot(page, "04_task_config_modal")

                # 填写任务名称
                task_name_input = page.locator("#taskName")
                if task_name_input.count() > 0:
                    task_name_input.fill(f"测试任务_{int(time.time())}")
                    print("  已填写任务名称")

                # 填写任务描述
                task_desc_input = page.locator("#taskDescription")
                if task_desc_input.count() > 0:
                    task_desc_input.fill("这是一个自动化测试任务")
                    print("  已填写任务描述")

                # 填写项目路径
                task_path_input = page.locator("#taskProjectPath")
                if task_path_input.count() > 0:
                    task_path_input.fill("/tmp/test_project")
                    print("  已填写项目路径")

                # 点击确认
                confirm_btn = page.locator("#taskConfigModal .btn-primary:has-text('确认配置')")
                if confirm_btn.count() > 0:
                    confirm_btn.click()
                    time.sleep(0.5)
                    print("  任务配置已保存")

            save_screenshot(page, "05_before_execute")

            # ========================================
            # 步骤6: 执行工作流
            # ========================================
            print("\n[步骤6] 执行工作流...")

            if btn_execute.is_visible():
                btn_execute.click()
                print("  已点击执行按钮")
                time.sleep(2)
                save_screenshot(page, "06_executing")

                # 检查按钮状态变化
                execute_visible_after = btn_execute.is_visible() if btn_execute.count() > 0 else False
                pause_visible_after = btn_pause.is_visible() if btn_pause.count() > 0 else False

                print(f"  执行后 - 执行按钮可见: {execute_visible_after}")
                print(f"  执行后 - 暂停按钮可见: {pause_visible_after}")

                if execute_visible_after:
                    issues.append("执行后执行按钮仍然可见")
                if not pause_visible_after:
                    issues.append("执行后暂停按钮不可见")

                # ========================================
                # 步骤7: 测试暂停功能
                # ========================================
                print("\n[步骤7] 测试暂停功能...")

                if btn_pause.is_visible():
                    time.sleep(1)  # 等待一下让执行进行

                    btn_pause.click()
                    print("  已点击暂停按钮")
                    time.sleep(2)
                    save_screenshot(page, "07_paused")

                    # 检查按钮状态
                    pause_visible_pause = btn_pause.is_visible() if btn_pause.count() > 0 else False
                    resume_visible_pause = btn_resume.is_visible() if btn_resume.count() > 0 else False

                    print(f"  暂停后 - 暂停按钮可见: {pause_visible_pause}")
                    print(f"  暂停后 - 继续按钮可见: {resume_visible_pause}")

                    if pause_visible_pause:
                        issues.append("暂停后暂停按钮仍然可见")
                    if not resume_visible_pause:
                        issues.append("暂停后继续按钮不可见")

                    # ========================================
                    # 步骤8: 测试恢复功能
                    # ========================================
                    print("\n[步骤8] 测试恢复功能...")

                    if btn_resume.is_visible():
                        btn_resume.click()
                        print("  已点击继续按钮")
                        time.sleep(2)
                        save_screenshot(page, "08_resumed")

                        # 检查按钮状态
                        pause_visible_resume = btn_pause.is_visible() if btn_pause.count() > 0 else False
                        resume_visible_resume = btn_resume.is_visible() if btn_resume.count() > 0 else False

                        print(f"  恢复后 - 暂停按钮可见: {pause_visible_resume}")
                        print(f"  恢复后 - 继续按钮可见: {resume_visible_resume}")

                        if not pause_visible_resume:
                            issues.append("恢复后暂停按钮不可见")
                        if resume_visible_resume:
                            issues.append("恢复后继续按钮仍然可见")

            # ========================================
            # 步骤9: 检查日志面板
            # ========================================
            print("\n[步骤9] 检查日志面板...")

            log_panel = page.locator("#logPanel")
            if log_panel.is_visible():
                print("  日志面板可见")

                # 检查执行日志
                exec_log_content = page.locator("#execLogContent")
                if exec_log_content.count() > 0:
                    log_text = exec_log_content.text_content()
                    print(f"  执行日志内容长度: {len(log_text) if log_text else 0}")

                # 检查日志数量徽章
                exec_log_count = page.locator("#execLogCount")
                if exec_log_count.count() > 0:
                    count_text = exec_log_count.text_content()
                    print(f"  执行日志数量: {count_text}")
            else:
                issues.append("日志面板不可见")

            save_screenshot(page, "09_logs")

            # ========================================
            # 步骤10: 再次检查历史记录
            # ========================================
            print("\n[步骤10] 再次检查历史记录...")

            history_btn = page.locator("button:has-text('历史')")
            if history_btn.count() > 0:
                history_btn.click()
                time.sleep(1)
                save_screenshot(page, "10_history_after_execute")

                history_list = page.locator("#historyList .history-item")
                new_count = history_list.count()
                print(f"  现在有 {new_count} 条历史记录")

                if new_count > 0:
                    # 点击第一条记录查看详情
                    first_history = history_list.first
                    first_history.click()
                    time.sleep(1)
                    save_screenshot(page, "11_history_detail")

                    # 检查详情内容
                    detail_content = page.locator("#historyDetailContent")
                    if detail_content.count() > 0:
                        detail_text = detail_content.text_content()
                        print(f"  详情内容已加载")

                        # 检查是否有操作按钮
                        action_btns = detail_content.locator(".log-action-btn")
                        print(f"  操作按钮数量: {action_btns.count()}")

            # ========================================
            # 检查控制台错误
            # ========================================
            print("\n[检查控制台消息]")

            errors = [m for m in console_messages if m['type'] == 'error']
            warnings = [m for m in console_messages if m['type'] == 'warning']

            print(f"  控制台错误: {len(errors)}")
            print(f"  控制台警告: {len(warnings)}")

            if errors:
                for err in errors[:5]:  # 只显示前5个
                    print(f"    错误: {err['text'][:100]}")
                issues.append(f"控制台有 {len(errors)} 个错误")

            if page_errors:
                for err in page_errors[:3]:
                    print(f"    页面错误: {err[:100]}")
                issues.append(f"页面有 {len(page_errors)} 个错误")

            # ========================================
            # 等待用户查看
            # ========================================
            print("\n测试完成，浏览器保持打开状态...")
            print("按 Ctrl+C 关闭浏览器")

            try:
                time.sleep(10)  # 给用户时间查看
            except KeyboardInterrupt:
                pass

        except Exception as e:
            print(f"\n测试异常: {e}")
            save_screenshot(page, "error_state")
            issues.append(f"测试异常: {str(e)}")

        finally:
            browser.close()

    # ========================================
    # 输出测试报告
    # ========================================
    print("\n" + "="*60)
    print("测试报告")
    print("="*60)

    if issues:
        print(f"\n发现 {len(issues)} 个问题:")
        for i, issue in enumerate(issues, 1):
            print(f"  {i}. {issue}")
    else:
        print("\n未发现明显问题")

    return issues

if __name__ == "__main__":
    issues = test_workflows_history()
    exit(0 if not issues else 1)